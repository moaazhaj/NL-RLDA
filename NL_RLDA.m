...
if (p < n)   
    ...
    for kk  = 1:length(gamma_vec)
            ...
            [em_hat,em_hat_p1,theta1_hat] = CECEP(n,p, gamma_vec(kk),S);  % Consistent Estimator of the Conditional Error Prob.           
    ...
    for kk = 1:length(gamma_vec)
            ...
            [em_hat,em_hat_p1,theta1_hat] = CECEP(n,p,gamma_vec(kk),D); % Consistent Estimator of the Conditional Error Prob.              
            T2       = em_hat_p1*(1+em_hat)*Dg11 + (1+em_hat)^2*Dg12;
            Dc       = gamma_vec(kk)^2*(1+em_hat)^4*Dg22-2*gamma_vec(kk)*T2+(1+em_hat)^2*Dg11;
            if (Dc>=0) 
                sqrtDc = sqrt(Dc);
