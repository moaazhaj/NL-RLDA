...
if (p < n)   
    ...
    for kk  = 1:length(gamma_vec)
            ...
            H11     =    inv(gamma_vec(kk)*eye(p) + S);            
            G0g     =    (m0 - 1/2*m_plus)'*H*m_minus;
            G1g     =    (m1 - 1/2*m_plus)'*H*m_minus;
            Dg11    =    m_minus'*H11*S*H11*m_minus;
            Dg22    =    m_minus'*H11^2*S*H11^2*m_minus;    
            Dg12    =    m_minus'*H11^2*S*H11*m_minus;            
            [em_hat,em_hat_p1,theta1_hat] = CECEP(n0,n1,p, gamma_vec(kk),S);  % Consistent Estimator of the Conditional Error Prob.           
            T2      =    em_hat_p1*(1+em_hat)*Dg11 + (1+em_hat)^2*Dg12;
            Dc    =    gamma_vec(kk)^2*(1+em_hat)^4*Dg22-2*gamma_vec(kk)*T2+(1+em_hat)^2*Dg11;
            if (Dc>=0) 
                sqrtDc = sqrt(Dc);
            else
                continue  % avoid the negative values of D
            end                 
    ...
    H                =  1/gamma_o^2*(inv(eye(p) + 1/gamma_o*S))^2*S;
           ...
           [~,D,V] = svd(X,'econ');
    for kk = 1:length(gamma_vec)
            Hb       = (D^2/n_tilde + gamma_vec(kk)*eye(n))^(-1);   
            H        = V*Hb^2*D^2*V'/n_tilde;   
            G0g      = (m0 - 1/2*m_plus)'*H*m_minus;
            G1g      = (m1 - 1/2*m_plus)'*H*m_minus;
            H22      = V*Hb^4*D^2*V'/n_tilde;
            H12      = V*Hb^3*D^2*V'/n_tilde;
            Dg11     = m_minus'*H*m_minus;
            Dg22     = m_minus'*H22*m_minus;    
            Dg12     = m_minus'*H12*m_minus;            
            [em_hat,em_hat_p1,theta1_hat] = CECEP_LC(n0,n1,p,gamma_vec(kk),D); % Consistent Estimator of the Conditional Error Prob.              
            T2       = em_hat_p1*(1+em_hat)*Dg11 + (1+em_hat)^2*Dg12;
            Dc       = gamma_vec(kk)^2*(1+em_hat)^4*Dg22-2*gamma_vec(kk)*T2+(1+em_hat)^2*Dg11;
            if (Dc>=0) 
                sqrtDc = sqrt(Dc);
